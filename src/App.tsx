              ...
              <>
                <div>
                  ...
                  <select value={formData.grade} onChange={(e) => setFormData({...formData, grade: e.target.value})} className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-blue-500 focus:outline-none transition text-lg">
                    {[5,6,7,8,9,10].map(g => (<option key={g} value={g}>{g}-сынып</option>))}
                  </select>
                </div>
                <div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <p className="text-2xl text-gray-600">Виртуалды лабораторияда тәжірибелерді қауіпсіз жасаңыз</p>
        </motion.div>
          ...
          {filtered.map((exp, i) => (
            <motion.div key={exp.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} whileHover={{ scale: 1.05, y: -10 }} onClick={() => onNavigate('experiment-detail', exp.id)} className="bg-white rounded-2xl shadow-2xl overflow-hidden cursor-pointer">
              <div className="bg-gradient-to-r from-blue-500 to-purple-500 p-8 text-white">
                ...
                <p className="text-lg opacity-90">{exp.subject === 'chemistry' ? 'Химия' : (exp.subject === 'biology' ? 'Биология' : 'Жаратылыстану')}</p>
              </div>
              <div className="p-6">
